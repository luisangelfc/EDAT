// Etapas, decisiones y notificaciones guardadas en localStorage. Requiere candidatos-db.js.

const EDAT_LLAVE_FLUJO = "edatFlujo";
const EDAT_LLAVE_SESION = "talentoSession";

// Las fechas de la demo se calculan a partir de esta fecha.
const EDAT_FECHA_BASE = "2026-09-24";

// Los estados coinciden con status_proceso de candidatos-db.js.
...
// Agrupa las etapas internas en las seis etapas visibles del proceso.
...
// Motivos de descarte basados en el perfil y los requisitos no negociables.
...
// Solo los perfiles con estado "En Proceso" entran al flujo activo.
...
function edatProcesoBloqueado() {
    ...
    if (edatEstado.vacante.estado === "Pendiente") {
        return edatResultado(false, "El proceso quedó en Pendiente y no puede continuar.");
    }
...
function edatEtapasVacante() {
    ...
    return EDAT_ETAPAS_VACANTE.map((etapa, posicion) => {
        ...
        if (indiceActual > fin || edatVacanteCerrada()) {
            estado = "completada";
        }
        ...
        if (edatEstado.vacante.estado === "Pendiente" && estado === "en-curso") {
            estado = "pendiente";
        }
...
function edatRevisarAlineacion() {
    ...
    if (at && hm) {
        ...
        // Al completar la alineación, los perfiles en proceso quedan disponibles.
...
function edatAtConfirmarIncorporacion() {
    ...
    function edatAtDeclinarProceso() {
        const bloqueo = edatProcesoBloqueado();
        if (bloqueo) {
            return bloqueo;
        }
        ...
        edatEstado.vacante.estado = "Pendiente";
        if (edatEstado.finalista) {
            edatFlujoCandidato(edatEstado.finalista).estado = "Pendiente";
        }
        edatGuardarEstado();
        return edatResultado(true, "Proceso declinado. Quedó en Pendiente.");
    }
...
// Busca al candidato por correo; si no coincide, usa el candidato del proceso activo.
...
const EDAT = {
    ...
    atDeclinarProceso: edatAtDeclinarProceso,
