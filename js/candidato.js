// Seguimiento del candidato y aceptación de ofertas. Requiere candidatos-db.js y edat-flow.js.
...
function candAbrirNotificaciones() {
    ...
    openModal(`
        ...
        <p>Actualizaciones de tu proceso en EDAT.</p>
        ${lista}
    `);
...
// TIMELINE
// ==========================================================
function candEtapasFlujo() {
    const internas = EDAT.etapasVacante();
    const porClave = Object.fromEntries(internas.map(etapa => [etapa.clave, etapa]));
    const busqueda = porClave.busqueda;
    const at = porClave.entrevistas_at;
    const hm = porClave.entrevistas_hm;
    const oferta = porClave.oferta;
    let seleccionado = "pendiente";
    if (
        at.estado === "completada"
        || at.estado === "en-curso"
        || at.estado === "detenida"
    ...
    else if (busqueda.estado === "en-curso" || busqueda.estado === "detenida") {
        seleccionado = busqueda.estado;
    }
    return [
        { clave: "seleccionado", nombre: "Seleccionado", numero: 1, estado: seleccionado },
        { clave: "entrevistas_at", nombre: "Entrevista AT", numero: 2, estado: at.estado },
        { clave: "entrevistas_hm", nombre: "Entrevista HM", numero: 3, estado: hm.estado },
        { clave: "oferta", nombre: "Oferta y cierre", numero: 4, estado: oferta.estado }
    ];
}
...
function candDetalleEtapa(clave, candidato) {
    ...
    if (candidato.estado === EDAT.ESTADOS.DESCARTADO) {
        return {
            eleccionado: "Tu perfil fue revisado para esta vacante.",
    ...
    if (candidato.estado === EDAT.ESTADOS.NO_SELECCIONADO) {
        return {
            seleccionado: "Fuiste seleccionado para continuar en el proceso.",
...
function candRenderTimeline() {
    ...
    if (!contenedor || !candidato) {
        ...
        contenedor.innerHTML = candEtapasFlujo()
        .map(etapa => {
...
(() => {
  ...
  const logout = document.getElementById('exitButton');
