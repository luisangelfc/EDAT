...
function renderTimeline() {
    ...
    timeline.innerHTML = etapas
        .map(etapa => {
            ...
            const tiempo = etapa.estado === "completada"
                ? ""
                : etapa.transcurrido >= etapa.sla
                    ? "overdue"
                    : etapa.transcurrido >= etapa.sla - 1
                        ? "due-soon"
                        : "on-time";

            return `
                <article class="timeline-stage ${clase} ${tiempo}">
...
function renderClosing() {
    ...
    if (estado.vacante.estado === "Pendiente") {
        acciones.innerHTML = "";
        return;
    }
    ...
    if (estado.oferta?.estado === "Aceptada") {
        ...
        acciones.innerHTML = `
            ...
            <button class="at-button at-button--ghost" type="button" data-decline-process>
                Declinar proceso
            </button>
...
function configureClosing() {
    ...
    el("atClosingActions").addEventListener("click", event => {
        ...
        if (event.target.closest("[data-decline-process]")) {
            runFlow(EDAT.atDeclinarProceso);
        }
...
function renderNavBadges() {
    ...
    const pendientesCierre = estado.oferta?.estado === "Aceptada" && !estado.incorporacion && estado.vacante.estado !== "Pendiente"
        ? 1
        : 0;
